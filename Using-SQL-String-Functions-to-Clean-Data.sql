...
-- In this task, I will create new tables; the customers
...
-- In this task, I will use the LENGTH function to return the 
-- length of a specified string, expressed as the number of characters.
-- In addition, I will use the LEFT/RIGHT functions to pull a certain number of characters 
...
SELECT LENGTH(first_name) AS characters_num, gender, first_name
FROM employees
WHERE gender = 'M';

-- 2.3: Find the length of the first name of male employees
-- where the length of the first name is greater than 5
SELECT LENGTH(first_name) AS character_num , first_name, gender
FROM employees
...
SELECT customer_name, region, country, REPLACE(country, 'United States', 'US') AS country_abb
FROM customers;

-- Task Five: TRIM, RTRIM, LTRIM
-- In this task, I will use the TRIM functions to remove
-- all specified characters either parts of a string

-- 5.1: Trim the word Munaf Ali Hashmi
SELECT TRIM(leading '' FROM ' Munaf Ali Hashmi')

-- 5.2: Trim the word Munaf Ali Hashmi
SELECT TRIM(trailing '' FROM ' Munaf Ali Hashmi ')

-- 5.3: Trim the word Munaf Ali Hashmi
SELECT TRIM(both ' ' FROM ' Munaf Ali Hashmi ');

-- 5.4: Trim the word Munaf Ali Hashmi
SELECT TRIM(' Munaf Ali Hashmi ');

-- 5.5: Right trim the word Munaf Ali Hashmi
SELECT RTRIM(' Munaf Ali Hashmi ');

-- 5.6: Left trim the word Munaf Ali Hashmi
SELECT LTRIM(' Munaf Ali Hashmi ');
...
SELECT customer_id, customer_name,
SUBSTRING(customer_id FROM 4 FOR 5) AS custnumber
FROM customers
WHERE SUBSTRING(customer_id FOR 2) = 'AB';
...
-- In this task, I will use string aggregation 
...
-- Task Nine: COALESCE
-- In this task, I will use COALESCE
