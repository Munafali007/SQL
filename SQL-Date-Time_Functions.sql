...
-- In this task, I will retrieve data from the tables in the
-- employees database
...
-- In this task, I will use a number of functions 
...
-- In this task, I will use the AGE() function to subtract arguments, 
...
SELECT * FROM employees;
SELECT first_name, last_name, birth_date, AGE(CURRENT_DATE, birth_date) AS current_age
FROM employees;


-- 3.4: Retrieve a list of all employees ages as at when they were employed
SELECT first_name, last_name, hire_date, AGE(CURRENT_DATE, hire_date) AS current_hiredate
FROM employees
...
SELECT e.first_name, e.last_name, s.salary, AGE(to_date, from_date) AS earned
FROM salaries s
...
-- In this task, I will use the EXTRACT() function 
...
SELECT product_id, sub_category, EXTRACT(MONTH FROM ship_date) AS sales_month
FROM sales
...
-- In this task, I will convert dates to strings
...
FROM sales
WHERE sub_category = 'Chairs' AND order_id LIKE '%2015%'
GROUP BY sales_month, full_sales_month
ORDER BY full_sales_month;
...
-- In this task, I will convert strings to dates
