...
-- Task One: 
-- In this task, I will retrieve data from tables in the employees database
...
-- Task Two: COUNT()
-- In this task, I will retrieve data from the employees
...
SELECT COUNT(salary) AS Num_of_Contracts
FROM salaries
WHERE salary >= 100000;
...
-- In this task, I will see the difference between SELECT DISTINCT
...
SELECT first_name,COUNT(first_name) AS D_FirstName
FROM employees
GROUP BY first_name
ORDER BY D_FirstName DESC
...
-- Task Four: HAVING
-- In this task, I will set conditions on the output of 
...
SELECT MAX(emp_no) AS highest_employee_no
FROM employees;
...
-- Task Eight: ROUND()
-- In this task, I will tidy up the result set from an 
-- aggregate function using ROUND(). In addition, I will perform some arithmetic
-- operations by combining different aggregate function

-- ROUND()

-- 8.1: Round the average salary to the nearest whole number
SELECT ROUND(AVG(salary)) AS averagge_salary
FROM salaries;

-- 8.2: Round the average salary to a precision of cents.
SELECT ROUND(AVG(salary), 2) AS averagge_salary
FROM salaries;
